...
public class ProgrammaticallyConfigYard implements ConfigYard {
	private final static Logger logger = LoggerFactory.getLogger(ProgrammaticallyConfigYard.class);
	/**
	 * 存储配置内容
	 */
	...
	public ProgrammaticallyConfigYard(String serverstring, ConfigYardCallBack callback) {
		this.client = new ZkClient(serverstring);
		configYardWatcher = new ConfigYardWatcher(client, this, callback);
		this.init();
	}
	...
	public void update(String key, String value) {
		String contactKey = this.contactKey(key);
		System.out.println("zk update start ...<" + contactKey + ">" + value);
		this.client.writeData(contactKey, value);
		System.out.println("zk update end ...");
		configYardWatcher.watcher(contactKey);
	}
