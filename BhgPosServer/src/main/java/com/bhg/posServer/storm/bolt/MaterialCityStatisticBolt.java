...
public class MaterialCityStatisticBolt extends AbstractVisitStatisticBolt {

	private static final long serialVersionUID = 1L;

	@Override
	protected boolean keyFieldExist(Tuple input) {
		return input.contains(StormConstant.BOLT_FIELD_MATERIAL_ID) && input.contains(StormConstant.BOLT_FIELD_CITY);
	}

	@Override
	protected String redisPVClickField(Tuple input) {
		return redisTime(input) + "#" + input.getStringByField(StormConstant.BOLT_FIELD_MATERIAL_ID) + "#" + input.getIntegerByField(StormConstant.BOLT_FIELD_CITY);
	}

	@Override
	protected String redisUVKey(Tuple input) {
		return Constants.REDIS_KEY_MATERIAL_CITY_UV + "#" + redisPVClickField(input);
	}
	...
	protected String redisPVKey() {
		return Constants.REDIS_KEY_MATERIAL_CITY_PV;
	}

	@Override
	protected String redisClickKey() {
		return Constants.REDIS_KEY_MATERIAL_CITY_CLICK;
	}
